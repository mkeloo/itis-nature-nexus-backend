  ...
  try {
    ...
    const sql = `
          ...
          SELECT
              *,
              CASE
                  WHEN precipitation > (SELECT AVG(precipitation) FROM climate_summary) THEN 'High Precipitation'
                  WHEN avg_temperature > (SELECT AVG(avg_temperature) FROM climate_summary) THEN 'High Temperature'
